...
class Special extends Controller
{
    ...
    public function indepNotify()
    {
        $uid = input('param.uid');
        $params = input('param.');
        $orderType = input('orderType', 'mall', 'trim');
        apilog($uid, 'mobile', 'alipaynotify-order', '', $params, '');
        if ($orderType == 'mall') {
        ...
        if ($orderType == 'recharge') {
            $notify = new \alipay\Notify();
            $order = db(\tname::agent_recharge)->where('order_sn', $params['out_trade_no'])->find();
            $params['out_trade_no'] = $order['order_sn'];
            $params['total_amount'] = $order['pay_money'];
            $refult = $notify::check($params);
            echo $refult;
            if ($params['trade_status'] == 'TRADE_SUCCESS') {
                $AgentLogic = new \app\common\logic\AgentLogic();
                $AgentLogic->rechargePaySuccess($order, 'alipay', $params['trade_no']);
            }
        }
