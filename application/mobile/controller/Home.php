...
//        dump($result);
    ...
    public function detail()
    {
        ...
        //全部评价
        $comment[0] = db(\tname::mall_comment)->where(array('product_id' => $goods_id, 'is_show' => 1))->count();
        //好评
        $comment[1] = db(\tname::mall_comment)->where(array('product_id' => $goods_id, 'is_show' => 1, 'star' => ['gt', 3]))->count();
        //中评
        $comment[2] = db(\tname::mall_comment)->where(array('product_id' => $goods_id, 'is_show' => 1, 'star' => ['eq', 3]))->count();
        //差评
        $comment[3] = db(\tname::mall_comment)->where(array('product_id' => $goods_id, 'is_show' => 1, 'star' => ['lt', 3]))->count();
