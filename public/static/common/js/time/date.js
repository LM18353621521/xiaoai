...
(function ($) {      
	...
	function resetIndex(){
        ...
        function resetInitTime(){
            ...
            if(that.val()!=""){
            	var times=new Array();
            	times=that.val().split(' ');
            	var time=times[1];
            	var ta=new Array();
            	ta=String(time).split(':');
            	var ihour=ta[0];
            	var imin=ta[1]
            	 if(ihour>12){    //判断当前时间是上午还是下午
                     SecondScroll.scrollTo(0, initD*40-40, 100, true);   //显示“下午”
                     ihour=ihour-12-1;
                }else{
                	ihour=ihour-1;
                }
            	 initH = parseInt(ihour);
            	 initI = parseInt(imin);
             }
             else if(that.val()===""){return false;}
        ...
        function bindButton(){
            ...
            $("#dateconfirm").unbind('click').click(function () {	
			  ...
			  $("#daywrapper ul li:eq("+Math.round(indexD)+")").html().substr(0,$("#daywrapper ul li:eq("+Math.round(indexD)+")").html().length-1);
               ...
               //将字符串转换为日期
               var begin=new Date();
               var end=new Date(datestr.replace(/-/g,"/"));
               //js判断日期
               if(begin-end>0){
                  createUL()
                  global_obj.win_alert("出发时间不可晚于当前时间");  
                  return false;
               }
