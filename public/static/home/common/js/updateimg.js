 $(function () {
        ...
        function reviewFile(file,_this) {
            ...
            fd.onload = function () {
                var goods_id = _this.parents('.discuss-list').find('.goods_id').val();
                if (/^image\/[jpeg|png|jpg|gif]/.test(fileType)) {
                    _this.parent().prev('.file-list').append('<li class="file-item"><input type="hidden" name="comment['+goods_id+'][imgs][]" value="' + this.result + '"><img class="img-pic" src="' + this.result + '" alt=""><span class="file-del">删除</span></li>').children(':last').hide().fadeIn(1000);
                } else {
